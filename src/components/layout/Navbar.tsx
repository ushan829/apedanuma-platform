...
export default function Navbar() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.authenticated && data.user) {
            ...
            setUserRole(data.user.role as "student" | "admin");
          } else {
            setIsLoggedIn(false);
            setUserName("");
            setUserRole("");
  ...
  }, [pathname]); // re-run on every route change — reacts immediately after login/logout
      ...
      {isOpen && (
        ...
        >
          <div className="container-xl py-4 pb-6">
            ...
            {/* Auth actions */}
            <div className="flex flex-col gap-3 px-1" style={{ animation: "navLinkFadeIn 280ms 180ms ease both" }}>
              {isLoggedIn ? (
                <>
                  {/* User info strip */}
                  <div
