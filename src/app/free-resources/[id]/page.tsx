...
/* ─────────────────────────────────────────
   Document cover — replaces the fake PDF viewer.
   Shows a clean document card so students know they
   need to download to read the actual content.
   ───────────────────────────────────────── */
function DocumentCover({ resource }: { resource: LiveResource }) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(90deg, transparent, ${s.color}55 40%, ${s.color}33 60%, transparent)`,
        }}
      />

      {/* Subject colour header band */}
      <div
        className="px-6 py-4 flex items-center justify-between shrink-0"
        style={{
          background: `linear-gradient(135deg, ${s.bg} 0%, rgba(10,10,10,0.85) 100%)`,
          borderBottom: `1px solid ${s.border}`,
        }}
      >
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full" style={{ background: s.color }} />
          <span className="text-[0.6rem] font-bold uppercase tracking-widest" style={{ color: s.color }}>
            {resource.subject}
          </span>
        </div>
        <span
      ...
      <div className="flex flex-col items-center justify-center flex-1 gap-6 px-6 py-10">
        ...
        {/* PDF file icon */}
        <div
          className="relative flex items-center justify-center w-24 h-28 rounded-xl shrink-0"
          style={{
            background: `linear-gradient(160deg, ${s.bg} 0%, rgba(10,10,10,0.9) 100%)`,
            border: `1px solid ${s.border}`,
            boxShadow: `0 0 32px ${s.color}22, 0 8px 24px rgba(0,0,0,0.4)`,
          }}
        >
          {/* Folded corner */}
          <div
            ...
            style={{
              width: 22, height: 22,
              background: `linear-gradient(225deg, rgba(10,10,10,0.95) 50%, ${s.bg} 50%)`,
              borderLeft: `1px solid ${s.border}`,
              borderBottom: `1px solid ${s.border}`,
              borderBottomLeftRadius: 4,
            }}
          />
          <svg width="36" height="36" viewBox="0 0 36 36" fill="none" aria-hidden="true" style={{ color: s.color }}>
            ...
            <path d="M13 16h10M13 20h10M13 24h6" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
          </svg>
        </div>

        {/* Title */}
        <div className="text-center space-y-1.5 max-w-sm">
          <h2 className="font-display font-semibold text-base leading-snug" style={{ color: "var(--foreground)" }}>
            {resource.title}
          </h2>
        ...
        {/* Notice */}
        <div
          className="flex items-start gap-3 rounded-xl px-4 py-3 text-left max-w-sm w-full"
          style={{
            background: "rgba(124,31,255,0.07)",
            border: "1px solid rgba(124,31,255,0.18)",
          }}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="shrink-0 mt-0.5" style={{ color: "#9455ff" }}>
            <circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.4" />
            <path d="M8 7v4M8 5v.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
          <p className="text-xs leading-relaxed" style={{ color: "var(--foreground-secondary)" }}>
            Download the PDF to read this resource. The file opens in any PDF viewer — no special software needed.
          </p>
        </div>
      ...
      >
        <span className="text-[0.65rem]" style={{ color: "var(--foreground-muted)" }}>
          PDF Document
          {resource.pageCount ? ` · ${resource.pageCount} pages` : ""}
          {resource.fileSize  ? ` · ${resource.fileSize}`        : ""}
        </span>
